<div id="page-content">
  ...
  <div class="content-header">
    <div class="header-section">
      <h1>
        <i class="gi gi-film"></i>Campaigns <br><small>Influencer</small>
      </h1>
    </div>
  </div>
...
function sort_campaigns() {
  ...
  var url=siteurl+"influencer/campaigns_ajax";

  var category=$('ul.nav-pills-category').find('li.active').text();
  $.post(url,{cat:category, id:'<?php echo $this->influencer_model->get_id(); ?>', seg:segment}, function(data) {
    //console.log(data);
    if(data!=false){
      ...
      $.each(data, function(key, obj) {
        ...
        html+='  <div class="widget" style="border: 2px solid #1BBAE1;border-radius: 5px;">';
        ...
        html+='          <div class="col-xs-4"><h3><small><button type="button" class="btn btn-primary" onclick="view_campaign('+obj.id+')">VIEW</button></small></h3></div>';
        html+='        </div>';
        html+='      </div>';
        html+='    </div>';
        html+='  </div>';
